...
//   node scripts/telegram-app.mjs describe                name, description, short description (zh-TW)
...
//   node scripts/telegram-app.mjs say "/status" [--user ID] [--chat ID] [--relay URL]   forge an Update as that person (webhook secret) —
//   node scripts/telegram-app.mjs tap "pb:parley:wire" [--user ID] [--chat ID]          the relay answers in the REAL Telegram chat
...
if (what === "show") {
  ...
  if (!payload) { console.error(`usage: telegram-app.mjs ${what} "<text or callback data>" [--user ID] [--chat ID]`); process.exit(2); }
  const userId = Number(args.includes("--user") ? args[args.indexOf("--user") + 1] : env.TELEGRAM_TEST_USER_ID);
  ...
  const chatId = Number(args.includes("--chat") ? args[args.indexOf("--chat") + 1] : userId);
  ...
  const bot = { id: Number(TOKEN.split(":")[0]), is_bot: true, first_name: "can2cup", username: env.TELEGRAM_BOT_USERNAME ?? "can2cup_bot" };
